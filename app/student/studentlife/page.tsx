...
async function getLifeCards(): Promise<LifeCard[]> {
  try {
    const res = await fetch(`${API_BASE}/api/student-life-cards`, { next: { revalidate: 3600 } });
    if (!res.ok) return [];
...
async function getSupportCards(): Promise<SupportCard[]> {
  try {
    const res = await fetch(`${API_BASE}/api/student-support`, { next: { revalidate: 3600 } });
    if (!res.ok) return [];
    const data = await res.json();
    return Array.isArray(data) ? data : data.data ?? [];
  } catch {
...
export default async function StudentLifePage() {
  const [lifeCards, supportCards] = await Promise.all([getLifeCards(), getSupportCards()]);
    ...
    <>
      ...
      <section className="py-16 px-5 bg-[#f8f9fc]">
        <div className="max-w-[1280px] mx-auto">
          ...
          {lifeCards.length === 0 ? (
            <p className="text-center text-gray-400 text-sm py-10">No content available.</p>
          ) : (
