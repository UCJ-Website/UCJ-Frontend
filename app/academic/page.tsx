...
async function getDepartments(): Promise<Department[]> {
  try {
    ...
    return json?.data?.departments?.data ?? [];
  } catch {
    return [];
  }
}
...
async function getUnits(): Promise<Unit[]> {
  try {
    const res = await fetch(`${API_BASE}/api/units`, { cache: "no-store" });
    if (!res.ok) return [];
    const json = await res.json();
    return json?.data?.units?.data ?? [];
  } catch {
    return [];
  }
}

function EmptyState({ icon = "fa-circle-info", title = "No data found", desc = "Please check back later." }: {
...
export default async function AcademicPage() {
  ...
  const engDepts  = departments.filter((d) => !d.is_unit && d.is_engineering);
  const nengDepts = departments.filter((d) => !d.is_unit && !d.is_engineering);

  // Units: exclude board-of-managment & board-of-studies (they're in About)
  const displayUnits = units.filter(
    (u) => u.slug !== "board-of-managment" && u.slug !== "board-of-studies"
  );
        ...
        {departments.length === 0 ? (
          ...
          <>
            ...
            {engDepts.length > 0 ? (
              ...
              <EmptyState icon="fa-cogs" title="No engineering departments" desc="No engineering departments to display." />
            )}
            ...
            {nengDepts.length > 0 ? (
              ...
              <EmptyState icon="fa-graduation-cap" title="No non-engineering departments" desc="No non-engineering departments to display." />
            )}
          </>
        )}

        {/* Units — Dynamic from API */}
        <div className="mt-16">
          ...
          {displayUnits.length === 0 ? (
            <EmptyState icon="fa-school" title="No units found" desc="No academic units to display right now." />
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[18px]">
              {displayUnits.map((unit) => (
                <Link
                  key={unit.id}
                  href={`/academic/units/${unit.slug}`}
                ...
                >
                  <div className="w-[42px] h-[42px] rounded-[10px] bg-[#f0f4ff] text-[#2563b0] flex items-center justify-center text-[18px] shrink-0">
                    <i className={`fas ${getIcon(unit.name)}`}></i>
                  </div>
                  <div>
                    <div className="font-semibold text-[#0f2a5e] text-[14px] mb-1">{unit.name}</div>
                    <div className="text-[12px] text-[#6b7280] leading-[1.5]">{unit.short_description}</div>
                  </div>
                </Link>
              ))}
            </div>
